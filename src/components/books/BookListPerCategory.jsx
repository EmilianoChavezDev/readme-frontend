        ...
        <div className='flex flex-col gap-2'>
            ...
            <div className='flex justify-center items-center py-3 px-60 cursor-pointer' onClick={() => router.push(`/books/${books?.filter(book => book.categoria === category)[0]?.id}`)}>
                <div className='flex gap-5 p-3 border border-gray-300 rounded-md'>
                    {books[0]?.portada?
                        <img src={books[0]?.portada} className='object-cover min-w-56 max-w-56 aspect-portada' alt='Portada de Libro' /> :
                    ...
                    <div className='flex flex-col justify-center gap-3 min-w-96'>
                        ...
                        <p>{books[0]?.sinopsis?.length > MAX_SINOPSIS_LENGTH? `${books[0]?.sinopsis.substring(0, MAX_SINOPSIS_LENGTH)}...` : books[0]?.sinopsis}</p>
                    </div>
                </div>
            </div>
            <ScrollableBookList>

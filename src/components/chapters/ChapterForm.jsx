...
export default function ChapterForm ({ bookId, chapter, onSave, onPublish }) {
    ...
    const handleSave = () => {
        if (!chapterTitle?.trim()?.length) return toast.error('El título del capítulo es requerido')
        if (!chapterContent?.trim()?.length) return toast.error('Debe agregar algún contenido antes de poder guardar')
        onSave(getFormData())
    }

    const handlePublish = () => {
        if (!chapterTitle?.trim()?.length) return toast.error('El título del capítulo es requerido')
        if (!chapterContent?.trim()?.length) return toast.error('Debe agregar algún contenido antes de poder publicar')

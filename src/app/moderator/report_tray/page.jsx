...
export default function Page() {
  ...
  const handleApproveReport = async () => {
    let params = {
      estado: reportSelected.estado,
      nuevo_estado: "resuelto",
      conclusion: reportConclusion,
    };
    const result =
      reportSelected.tipo === "Usuario"
        ? await updateAllUserReport({
            ...params,
            usuario_reportado_id: reportSelected?.usuario_reportado?.id,
          })
        : reportSelected.tipo === "Comentario"
        ? await updateAllCommentReport({
            ...params,
            comentario_id: reportSelected?.comentario?.id,
          })
        : await updateAllBookReport({
            ...params,
            libro_id: reportSelected?.libro_id,
          });
    if (result?.error) {
      ...
      if (reportSelected.tipo === "Usuario") {
        const userResult = await deleteUser(
          reportSelected?.usuario_reportado?.id
        );
    ...
    <>
              ...
              <>
                ...
                <div className="flex gap-1 items-center">
                  ...
                  {reportSelected?.estado === "rechazado" ? (
                    ...
                    <>
                      <button
                        onClick={() => handleUpdateReportStatus("pendiente")}
                        className={`h-9 px-2 rounded-2xl flex items-center hover:brightness-90 ${
                          reportSelected?.estado === "pendiente"
                            ? "bg-colorPrimario text-white"
                            : "border border-colorPrimario text-colorPrimario"
                        }`}
                      >
                        Pendiente
                      </button>
                      <button
                        onClick={() => handleUpdateReportStatus("en_revision")}
                        className={`h-9 px-2 rounded-2xl flex items-center hover:brightness-90 ${
                          reportSelected?.estado === "en_revision"
                            ? "bg-colorPrimario text-white"
                            : "border border-colorPrimario text-colorPrimario"
                        }`}
                      >
                      ...
                      <button
                        onClick={() => handleUpdateReportStatus("aceptado")}
                        className={`h-9 px-2 rounded-2xl flex items-center hover:brightness-90 ${
                ...
                <div className="border border-gray-300 rounded-md p-3 flex flex-col gap-3">
                  <span className="font-semibold text-lg">{`Reporte de ${
                    reportSelected?.tipo ?? ""
                  }`}</span>
                  ...
                  {Boolean(reportSelected?.usuario_reportado) && (
                    <div className="flex gap-1">
                      ...
                      <span className="text-gray-700">
                        {reportSelected?.usuario_reportado?.username}
                      </span>
                    </div>
                  )}
                  <div className="flex gap-1">
                ...
                <div className="flex justify-between text-white">
                  ...
                  {reportSelected?.tipo === "Usuario" && (
                    <Link
                      href={`/user/${reportSelected?.usuario_reportado?.username}`}
                    >
      ...
      >
        <div className="flex flex-col gap-3">
          <p>
            {reportSelected?.tipo === "Usuario"
              ? `¿Estás seguro de desactivar el perfil de ${reportSelected?.usuario_reportado?.username}?`
              : reportSelected?.tipo === "Comentario"
              ? `¿Estás seguro de eliminar el comentario \"${reportSelected?.comentario?.comentario}\" del libro ${reportSelected?.titulo_de_libro}?`

...
export default function Page() {
    ...
    const getCardPercentages = () => {
        let comments = (statistics?.cantidad_comentarios_ultima_semana * 100) / (statistics?.cantidad_total_comentarios - statistics?.cantidad_comentarios_ultima_semana)
        ...
        <>
            ...
            <div className='relative flex flex-col gap-3 px-20 py-9'>
                ...
                <div className='flex justify-between'>
                    <h1 className='font-bold text-gray-800 text-3xl leading-8'>Estadísticas</h1>
                ...
                <div className='flex gap-3'>
                    <div className='relative !max-w-40'>
                        {categories.length &&
                            <Select label='Categoría' className='!max-w-40'
                    ...
                    <button className='h-10 px-3 rounded-md bg-gray-400 hover:bg-gray-500 text-black' 
                        disabled={booksLoading} 
                        onClick={cleanSearchValues}>
                        Limpiar
                    </button>
                    <button className='h-10 px-3 rounded-md bg-colorPrimario hover:bg-colorHoverPrimario text-white' 
                        disabled={booksLoading}
                        onClick={() => fetchBooks()}>
                        Buscar
                    </button>
                    ...
                    <div className='grow flex flex-col gap-3'>
                        <div className='border rounded-md border-gray-300 w-full flex flex-col gap-6 items-center p-5'>
                            ...
                            <table className='w-full'>
                                ...
                                <tbody>{booksData?.data?.map((book, index) =>
                                    <tr key={index} 
                                        className={`h-14 hover:bg-green-100 cursor-pointer ${book.id === bookSelected.id? 
                                            'border-t border-t-gray-400 border-b-2 border-b-colorPrimario' : 
                                            'border-t border-gray-400'}`}
                                            onClick={() => handleSelectBook(book)}>
                                        <td className={`text-start font-normal ${book.id === bookSelected.id? 'text-colorPrimario font-semibold' : 'text-gray-800'}`}>
                                            {index + 1}
                                        </td>
                                        <td className={`text-start font-normal ${book.id === bookSelected.id? 'text-colorPrimario font-semibold' : 'text-gray-800'}`}>
                                            {book?.titulo}
                    ...
                    <div className='flex flex-col gap-3 !max-w-2/5'>
                        <div className='flex gap-3'>
                            <div className='border rounded-md border-gray-300 p-3 w-1/2 flex flex-col'>
                                <div className='flex justify-between'>
                                    <span className='text-xl font-bold'>{statistics?.cantidad_total_comentarios}</span>
                                ...
                                <div className='flex gap-1 items-center'>
                                    ...
                                    <span className='text-gray-500 text-sm'>
                                        {`${statistics?.cantidad_comentarios_ultima_semana} / ${getCardPercentages().comments}% esta semana`}
                                    </span>
                                </div>
                            </div>
                            <div className='border rounded-md border-gray-300 p-3 w-1/2 flex flex-col'>
                                <div className='flex justify-between'>
                                    <span className='text-xl font-bold'>{statistics?.cantidad_total_favoritos}</span>
                                ...
                                <div className='flex gap-1 items-center'>
                                    ...
                                    <span className='text-gray-500 text-sm'>
                                        {`${statistics?.cantidad_favoritos_ultima_semana} / ${getCardPercentages().favorites}% esta semana`}
                                    </span>
                                </div>
                            </div>
                        </div>
                        <div className='flex gap-3'>
                            <div className='border rounded-md border-gray-300 p-3 w-1/2 flex flex-col'>
                                ...
                                <div className='flex gap-1 items-center'>
                                    ...
                                    <span className='text-gray-500 text-sm'>
                                        {`${statistics?.cantidad_vistas_ultima_semana} / ${getCardPercentages().views}% esta semana`}
                                    </span>
                                </div>
                            </div>
                            <div className='border rounded-md border-gray-300 p-3 w-1/2 flex flex-col'>
                                <div className='flex justify-between'>
                                    <span className='text-xl font-bold'>{statistics?.edad_promedio? `${statistics?.edad_promedio} años` : 'Sin Datos'}</span>
